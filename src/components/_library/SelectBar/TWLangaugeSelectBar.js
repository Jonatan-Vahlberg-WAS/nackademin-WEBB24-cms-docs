    ...
    <SelectBar
      ...
      selectItem={(item) => {
        const language = languages.find((language) => {
          return useShort ? language.short === item.value : language.kind === item.value
        })
    ...
    >
      <>
        <div className='tw:!flex tw:!items-center tw:!gap-2'>
          {showAddLanguage && (
            <Button size='sm' variant='success' outline onClick={onAddLanguage} className='tw:!leading-none tw:!font-normal'>
              +&nbsp;
              {i18next.t('general.language.addLanguage')}
            </Button>
          )}
          {showRemoveLanguage && (
            <Button size='sm' variant='danger' outline onClick={onRemoveLanguage} className='tw:!leading-none tw:!font-normal'>
              -&nbsp;
