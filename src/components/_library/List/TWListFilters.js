...
const listFilters = {
  SearchFilter: ({
    ...
    const searchClasses = cn("pl-7", className);
  ...
  LanguageFilter: ({
    ...
    const wrapperClasses = cn("flex flex-wrap gap-2", className);
    return (
      <div className={wrapperClasses}>
        {languages.map((language) => {
          const isChecked =
            selectedLanguages.includes(language.short) ||
            (checkedIfEmpty && selectedLanguages.length === 0);
          return (
            <Checkbox
              label={language.name}
              checked={isChecked}
